...
struct __hashtable_iterator
{
  ...
  node *cur;     //指向当前节点
  hashtable *ht; //指向哈希表，类似deque的迭代器
...
struct __hashtable_const_iterator
{
  ...
  typedef ptrdiff_t difference_type;
  typedef size_t size_type;
  typedef const Value &reference; //常量迭代器使用常量引用
  typedef const Value *pointer;   //常量迭代器使用常量指针

  const node *cur;     //指向当前节点
  const hashtable *ht; //指向哈希表，类似deque的迭代器
...
public:
  ...
  typedef EqualKey key_equal; // key比较方法
...
private:
  hasher hash;        //哈希函数
  key_equal equals;   // key比较方法
  ExtractKey get_key; //从value中提取key

  typedef __hashtable_node<Value> node;
  typedef simple_alloc<node, Alloc> node_allocator;

  vector<node *, Alloc> buckets; //哈希桶
  size_type num_elements;        //记录元素个数
...
hashtable<V, K, HF, Ex, Eq, A>::insert_unique_noresize(const value_type &obj)
{
  ...
  for (node *cur = first; cur; cur = cur->next)
    if (equals(get_key(cur->val), get_key(obj))) //存在待插入元素，返回失败
...
hashtable<V, K, HF, Ex, Eq, A>::insert_equal_noresize(const value_type &obj)
{
  ...
  //遍历桶
  for (node *cur = first; cur; cur = cur->next)
    if (equals(get_key(cur->val), get_key(obj))) //如果存在待插入元素，头插法插入一个新值
    {
      node *tmp = new_node(obj);
      tmp->next = cur->next;
      cur->next = tmp;
...
hashtable<V, K, HF, Ex, Eq, A>::equal_range(const key_type &key)
{
  ...
  {
    if (equals(get_key(first->val), key)) //[begin,
    {
      for (node *cur = first->next; cur; cur = cur->next)
        if (!equals(get_key(cur->val), key)) //,end)
...
hashtable<V, K, HF, Ex, Eq, A>::erase(const key_type &key)
{
  ...
  if (first)
  {
    ...
    while (next)
    {
      if (equals(get_key(next->val), key)) //符合删除条件
      {
        //从链表中移除
        cur->next = next->next;
        delete_node(next);
