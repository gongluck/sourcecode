...
extern "C++"
{
  ...
  operator new(size_t s, size_t extra) // Usage ：Rep *p = new (extra) Rep;
  {
      ...
      check_realloc(basic_string::size_type s) const
  {
    s += sizeof(charT);
    rep()->selfish = false;
    return (rep()->ref > 1                     //多个共享者
            || s > capacity()                  //请求大小大于容量
            || Rep::excess_slop(s, capacity()) //使用大小的两倍小于强求大小
    );
  }
      ...
      find(const charT *s, size_type pos, size_type n) const
  {
    ...
    for (; xpos + n <= length(); ++xpos)
      if (traits::eq(data()[xpos], *s) && traits::compare(data() + xpos, s, n) == 0)
        return xpos;
    return npos;
  }
      ...
      find_first_not_of(const charT *s, size_type pos, size_type n) const
  {
    size_t xpos = pos;
    for (; xpos < length(); ++xpos)
      if (_find(s, data()[xpos], 0, n) == npos) //低效！另外实现一个_nfind应该能高效处理
        return xpos;
    return npos;
  }
