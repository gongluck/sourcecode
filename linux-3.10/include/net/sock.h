...
#else
static inline int mem_cgroup_sockets_init(struct mem_cgroup *memcg, struct cgroup_subsys *ss)
{
	return 0;
}
static inline void mem_cgroup_sockets_destroy(struct mem_cgroup *memcg)
{
}
#endif
/*
...
static inline __printf(2, 3) void SOCK_DEBUG(const struct sock *sk, const char *msg, ...)
{
}
#endif
...
{
	return (!is_a_nulls(sk->sk_nulls_node.next)) ? hlist_nulls_entry(sk->sk_nulls_node.next,
...
#define sk_refcnt_debug_inc(sk) \
	do                            \
	{                             \
	} while (0)
#define sk_refcnt_debug_dec(sk) \
	do                            \
	{                             \
	} while (0)
#define sk_refcnt_debug_release(sk) \
	do                                \
	{                                 \
	} while (0)
