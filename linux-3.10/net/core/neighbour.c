...
static __inline__ int neigh_max_probes(struct neighbour *n)
{
	...
	return (n->nud_state & NUD_PROBE) ? p->ucast_probes : p->ucast_probes + p->app_probes + p->mcast_probes;
}
...
{
	struct hh_cache *hh;
	void (*update)(struct hh_cache *, const struct net_device *, const unsigned char *) = NULL;
	...
	if (new != old)
	{
		...
		if (new &NUD_IN_TIMER)
			neigh_add_timer(neigh, (jiffies +
															((new &NUD_REACHABLE) ? neigh->parms->reachable_time : 0)));
		neigh->nud_state = new;
	}
...
{
	return NLMSG_ALIGN(sizeof(struct ndmsg)) + nla_total_size(MAX_ADDR_LEN)		 /* NDA_DST */
				 + nla_total_size(MAX_ADDR_LEN)																			 /* NDA_LLADDR */
				 + nla_total_size(sizeof(struct nda_cacheinfo)) + nla_total_size(4); /* NDA_PROBES */
}
