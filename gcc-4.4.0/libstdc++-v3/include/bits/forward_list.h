...
struct _Fwd_list_node_base // 单向列表节点基础结构
{
  // The type allocated by _Alloc cannot be this type, so we rebind
  typedef typename _Alloc::template rebind<_Fwd_list_node_base<_Alloc>>::other::pointer _Pointer;
  typedef typename _Alloc::template rebind<_Fwd_list_node_base<_Alloc>>::other::const_pointer _Const_pointer;

  _Pointer _M_next; // next指针
...
struct _Fwd_list_node : public _Fwd_list_node_base<_Alloc> // 单向列表节点
{
  typedef typename _Alloc::template rebind<_Fwd_list_node<_Tp, _Alloc>>::other::pointer _Pointer;

  template <typename... _Args>
  _Fwd_list_node(_Args &&...__args) : _Fwd_list_node_base<_Alloc>(), _M_value(std::forward<_Args>(__args)...) {}
 ...
 */
template <typename _Tp, typename _Alloc>
struct _Fwd_list_iterator // 单向列表迭代器
{
  ...
  explicit _Fwd_list_iterator(typename _Node_base::_Pointer __n) : _M_node(__n) {}
  ...
  typename _Node_base::_Pointer _M_node; // 指向节点元素
};
...
struct _Fwd_list_const_iterator
{
  ...
  explicit _Fwd_list_const_iterator(typename _Node_base::_Const_pointer __n) : _M_node(__n) {}

  _Fwd_list_const_iterator(const iterator &__iter) : _M_node(__iter._M_node) {}
 ...
 */
template <typename _Tp, typename _Alloc>
inline bool operator==(const _Fwd_list_iterator<_Tp, _Alloc> &__x, const _Fwd_list_const_iterator<_Tp, _Alloc> &__y)
{
 ...
 */
template <typename _Tp, typename _Alloc>
struct _Fwd_list_base // 单向列表基础结构
{
protected:
  typedef typename _Alloc::template rebind<_Tp>::other _Tp_alloc_type;
  typedef typename _Alloc::template rebind<_Fwd_list_node<_Tp, _Tp_alloc_type>>::other _Node_alloc_type;

  struct _Fwd_list_impl : public _Node_alloc_type // 当选列表实现
  {
    _Fwd_list_node_base<_Tp_alloc_type> _M_head; // 头节点

    _Fwd_list_impl() : _Node_alloc_type(), _M_head()
    {
    }

    _Fwd_list_impl(const _Node_alloc_type &__a) : _Node_alloc_type(__a), _M_head()
    {
    }
  };
...
public:
  ...
  _Fwd_list_base() : _M_impl()
  {
    this->_M_impl._M_head._M_next = 0;
  }

  _Fwd_list_base(const _Alloc &__a) : _M_impl(__a)
  {
    this->_M_impl._M_head._M_next = 0;
  }
  ...
  _Fwd_list_base(_Fwd_list_base &&__lst, const _Alloc &__a) : _M_impl(__a)
  {
    _Node_base::swap(this->_M_impl._M_head, __lst._M_impl._M_head);
  }

  _Fwd_list_base(_Fwd_list_base &&__lst) : _M_impl(__lst._M_get_Node_allocator())
  {
    _Node_base::swap(this->_M_impl._M_head, __lst._M_impl._M_head);
  }
...
protected:
  typename _Node::_Pointer _M_get_node() // 分配一个节点
  {
    return _M_get_Node_allocator().allocate(1);
  }

  template <typename... _Args>
  typename _Node::_Pointer _M_create_node(_Args &&...__args) // 创建节点
  {
    typename _Node::_Pointer __node = this->_M_get_node(); // 分配节点空间
    __try
    {
      _M_get_Node_allocator().construct(__node, std::forward<_Args>(__args)...); // 原地构造
      __node->_M_next = 0;
    }
    __catch(...)
    {
      this->_M_put_node(__node); // 销毁节点
  ...
  template <typename... _Args>
  typename _Node_base::_Pointer _M_insert_after(const_iterator __pos, _Args &&...__args);

  void _M_put_node(typename _Node::_Pointer __p)
  {
    _M_get_Node_allocator().deallocate(__p, 1); // 销毁节点
  }

  typename _Node_base::_Pointer _M_erase_after(typename _Node_base::_Pointer __pos);
  typename _Node_base::_Pointer _M_erase_after(typename _Node_base::_Pointer __pos, typename _Node_base::_Pointer __last);
};
...
class forward_list : private _Fwd_list_base<_Tp, _Alloc> // 单向列表
{
private:
...
public:
   ...
   */
  explicit forward_list(const _Alloc &__al = _Alloc()) : _Base(__al)
  {
  }
   ...
   *  @param  al    An allocator object.
   */
  forward_list(const forward_list &__list, const _Alloc &__al) : _Base(__list, __al)
  {
  }
   ...
   *  @param  al    An allocator object.
   */
  forward_list(forward_list &&__list, const _Alloc &__al) : _Base(std::forward<_Base>(__list), __al)
  {
  }
   ...
   *  the default value.
   */
  explicit forward_list(size_type __n) : _Base()
  {
   ...
   *  value.
   */
  forward_list(size_type __n, const _Tp &__value, const _Alloc &__al = _Alloc()) : _Base(__al)
  {
  ...
  forward_list(_InputIterator __first, _InputIterator __last, const _Alloc &__al = _Alloc()) : _Base(__al)
  {
   ...
   */
  forward_list(const forward_list &__list) : _Base(__list.get_allocator())
  {
   ...
   *  %forward_list.
   */
  forward_list(forward_list &&__list) : _Base(std::forward<_Base>(__list)) {}
   ...
   */
  forward_list(std::initializer_list<_Tp> __il, const _Alloc &__al = _Alloc()) : _Base(__al)
  {
  ...
  iterator begin()
  {
    return iterator(this->_M_impl._M_head._M_next); // 第一个有效节点的迭代器
  }
  ...
  void insert_after(const_iterator __pos, size_type __n, const _Tp &__val)
  {
    ...
    this->splice_after(__pos, std::move(__tmp)); // 拼接
  }
...
template <typename _Tp, typename _Alloc>
inline void swap(forward_list<_Tp, _Alloc> &__lx, forward_list<_Tp, _Alloc> &__ly)
{
  __lx.swap(__ly);
}
