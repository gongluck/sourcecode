...
public:
  class value_compare : public std::binary_function<value_type, value_type, bool> // kv比较类
  {
...
private:
  ...
  typedef typename _Alloc::template rebind<value_type>::other_Pair_alloc_type;
...
public:
   ...
   */
  map(map &&__x) : _M_t(std::forward<_Rep_type>(__x._M_t)) {}
   ...
   */
  map(initializer_list<value_type> __l, const _Compare &__c = _Compare(), const allocator_type &__a = allocator_type()) : _M_t(__c, __a)
  {
