...
template <typename _InputIterator> // 求两输入迭代器的距离
...
template <typename _RandomAccessIterator> // 求两可随机访问迭代器的距离
 ...
 */
template <typename _InputIterator> // 求两迭代器的距离
...
template <typename _InputIterator, typename _Distance> // 递进输入迭代器
...
template <typename _BidirectionalIterator, typename _Distance> // 递进双向迭代器
...
template <typename _RandomAccessIterator, typename _Distance> // 递进可随机访问迭代器
 ...
 *  and are constant time.  For other %iterator classes they are linear time.
 */
template <typename _InputIterator, typename _Distance> // 递进迭代器
...
inline _InputIterator next(_InputIterator __x, typename iterator_traits<_InputIterator>::difference_type __n = 1)
{
  std::advance(__x, __n);
  return __x;
}
