 ...
 *  operations.
 */
template <typename _Tp, typename _Sequence = deque<_Tp>> // 栈 默认使用双端队列作为底层容器
class stack
{
...
public:
  ...
  explicit stack(_Sequence &&__c = _Sequence()) : c(std::move(__c)) {}
#endif
...
#ifdef __GXX_EXPERIMENTAL_CXX0X__
template <typename _Tp, typename _Seq>
inline void swap(stack<_Tp, _Seq> &__x, stack<_Tp, _Seq> &__y)
{
  __x.swap(__y);
}
