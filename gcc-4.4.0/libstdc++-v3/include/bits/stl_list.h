...
struct _List_node_base // 列表节点基础结构
{
...
struct _List_node : public _List_node_base
{
  ...
  _List_node(_Args &&...__args) : _List_node_base(), _M_data(std::forward<_Args>(__args)...)
  {
  }
#endif
};
...
struct _List_iterator // 列表迭代器
{
  typedef _List_iterator<_Tp> _Self;
  typedef _List_node<_Tp> _Node;
  typedef ptrdiff_t difference_type;
  typedef std::bidirectional_iterator_tag iterator_category; // 双向迭代器类别
  ...
  bool operator==(const _Self &__x) const // 迭代器相等判断
  {
    return _M_node == __x._M_node;
  }

  bool operator!=(const _Self &__x) const // 迭代器不等判断
  {
    return _M_node != __x._M_node;
  }
...
protected:
  ...
  _List_impl _M_impl; // 列表实现

  _List_node<_Tp> *_M_get_node() // 创建节点
  {
    return _M_impl._Node_alloc_type::allocate(1);
  }

  void _M_put_node(_List_node<_Tp> *__p) // 销毁节点
  {
...
public:
...
#ifdef __GXX_EXPERIMENTAL_CXX0X__
  _List_base(_List_base &&__x) : _M_impl(__x._M_get_Node_allocator())
  {
    _M_init();
  ...
  void _M_init()
  {
    this->_M_impl._M_node._M_next = &this->_M_impl._M_node; // 头节点next指向自身
    this->_M_impl._M_node._M_prev = &this->_M_impl._M_node; // 头节点prev指向自身
  }
};
...
class list : protected _List_base<_Tp, _Alloc> // 列表
{
...
protected:
  ...
  _Node *_M_create_node(const value_type &__x) // 创建节点
  {
    _Node *__p = this->_M_get_node(); // 创建内存空间
    __try
    {
      _M_get_Tp_allocator().construct(&__p->_M_data, __x); // 原地构造
    }
    __catch(...)
    {
      _M_put_node(__p); // 销毁空间
...
public:
   ...
   */
  explicit list(size_type __n, const value_type &__value = value_type(), const allocator_type &__a = allocator_type()) : _Base(__a)
  {
   ...
   */
  list(list &&__x) : _Base(std::forward<_Base>(__x)) {}
   ...
   */
  list(initializer_list<value_type> __l, const allocator_type &__a = allocator_type()) : _Base(__a)
  {
  ...
  iterator begin()
  {
    return iterator(this->_M_impl._M_node._M_next); // 头节点的next才是第一个有效节点
  }
  ...
  {
    list __tmp(__n, __x, _M_get_Node_allocator()); // 临时列表
    splice(__position, __tmp);                     // 拼接列表
  }
...
#endif
  {
    _List_node_base::swap(this->_M_impl._M_node, __x._M_impl._M_node); // 交换哨兵节点
