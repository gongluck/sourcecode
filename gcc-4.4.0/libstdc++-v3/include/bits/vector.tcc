...
void vector<_Tp, _Alloc>::reserve(size_type __n) // 存储大小保证
{
  ...
  if (this->capacity() < __n)
  {
    ...
    pointer __tmp = _M_allocate_and_copy(__n, _GLIBCXX_MAKE_MOVE_ITERATOR(this->_M_impl._M_start), _GLIBCXX_MAKE_MOVE_ITERATOR(this->_M_impl._M_finish)); // T tmp = T1;
    std::_Destroy(this->_M_impl._M_start, this->_M_impl._M_finish, _M_get_Tp_allocator());                                                                //~T1
    _M_deallocate(this->_M_impl._M_start, this->_M_impl._M_end_of_storage - this->_M_impl._M_start);
    this->_M_impl._M_start = __tmp; // T1 = tmp;
...
void vector<_Tp, _Alloc>::emplace_back(_Args &&...__args) // 原地构造插入
{
