 ...
 */
inline size_t __deque_buf_size(size_t __size) // 决定每个控制块存放多少个节点
{
...
struct _Deque_iterator
{
  ...
  {
    ++_M_cur;
    if (_M_cur == _M_last) // 当前控制块溢出
    {
      _M_set_node(_M_node + 1); // 使用下一个控制块
      _M_cur = _M_first;        // 重新标记到开始
    }
    return *this;
  }
...
protected:
  ...
  _Tp *_M_allocate_node() // 分配一个控制块的空间
  {
...
{
  const size_t __num_nodes = (__num_elements / __deque_buf_size(sizeof(_Tp)) + 1); // 计算需要多少个控制块
...
template <typename _Tp, typename _Alloc>
void _Deque_base<_Tp, _Alloc>::_M_create_nodes(_Tp **__nstart, _Tp **__nfinish) // 为控制块创建节点
{
  _Tp **__cur;
  __try
  {
...
public:
   ...
   */
  explicit deque(size_type __n, const value_type &__value = value_type(), const allocator_type &__a = allocator_type()) : _Base(__a, __n)
  {
   ...
   *  by @a x.
   */
  deque(const deque &__x) : _Base(__x._M_get_Tp_allocator(), __x.size())
  {
...
protected:
  ...
  void _M_erase_at_end(iterator __pos) // 销毁[pos,end)
  {
    _M_destroy_data(__pos, end(), _M_get_Tp_allocator());                     // 销毁元素
    _M_destroy_nodes(__pos._M_node + 1, this->_M_impl._M_finish._M_node + 1); // 销毁控制块
