 ...
 */
template <typename _Iterator> // 反向迭代器
...
{
protected:
  _Iterator current; // 指向当前元素的正向迭代器
...
public:
   ...
   *  @doctodo
   */
  reference operator*() const // 反向迭代器解引用取值是当前指向的前一个
  {
   ...
   *  @doctodo
   */
  reverse_iterator &operator++() // 反向递增
  {
    --current;
    return *this;
  }
...
class back_insert_iterator /*尾部插入迭代器*/ : public iterator<output_iterator_tag, void, void, void, void>
{
protected:
  _Container *container; // 容器指针
...
public:
  ...
  {
    container->push_back(__value); // 尾部插入
    return *this;
  }
 ...
 */
template <typename _Container>
class front_insert_iterator /*头部插入迭代器*/ : public iterator<output_iterator_tag, void, void, void, void>
{
protected:
 ...
 */
template <typename _Container>
class insert_iterator /*插入迭代器*/ : public iterator<output_iterator_tag, void, void, void, void>
{
protected:
  _Container *container;              // 指向容器
  typename _Container::iterator iter; // 当前指向迭代器
...
public:
   ...
   *  @endcode
   */
  insert_iterator &operator=(typename _Container::const_reference __value) // 赋值运算符做插入操作
  {
...
class __normal_iterator // 常规迭代器
{
protected:
  _Iterator _M_current; // 指向当前元素
...
public:
  ...
  template <typename _Iter>
  __normal_iterator(const __normal_iterator<_Iter, typename __enable_if<(std::__are_same<_Iter, typename _Container::pointer>::__value), _Container>::__type> &__i) : _M_current(__i.base()) {}
...
#ifdef __GXX_EXPERIMENTAL_CXX0X__
// DR 685.
inline auto operator-(const __normal_iterator<_IteratorL, _Container> &__lhs, const __normal_iterator<_IteratorR, _Container> &__rhs) -> decltype(__lhs.base() - __rhs.base())
#else
...
template <typename _Iterator, typename _Container>
inline __normal_iterator<_Iterator, _Container> operator+(typename __normal_iterator<_Iterator, _Container>::difference_type__n, const __normal_iterator<_Iterator, _Container> &__i)
{
...
template <typename _IteratorL, typename _IteratorR>
inline bool operator==(const move_iterator<_IteratorL> &__x, const move_iterator<_IteratorR> &__y)
{
  return __x.base() == __y.base();
}
...
// DR 685.
template <typename _IteratorL, typename _IteratorR>
inline auto operator-(const move_iterator<_IteratorL> &__x, const move_iterator<_IteratorR> &__y) -> decltype(__x.base() - __y.base())
{
  return __x.base() - __y.base();
}

template <typename _Iterator>
inline move_iterator<_Iterator> operator+(typename move_iterator<_Iterator>::difference_type __n, const move_iterator<_Iterator> &__x)
{
  return __x + __n;
}

template <typename _Iterator>
inline move_iterator<_Iterator> make_move_iterator(const _Iterator &__i)
{
