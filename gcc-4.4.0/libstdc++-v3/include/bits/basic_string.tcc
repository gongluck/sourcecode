...
_CharT *basic_string<_CharT, _Traits, _Alloc>::_S_construct(_InIterator __beg, _InIterator __end, const _Alloc &__a, input_iterator_tag)
{
  ...
  if (__beg == __end && __a == _Alloc()) // 空字符串
    return _S_empty_rep()._M_refdata();
#endif
  // Avoid reallocation for common case.
  _CharT __buf[128];
  size_type __len = 0;
  while (__beg != __end && __len < sizeof(__buf) / sizeof(_CharT)) // 前128则拷贝到栈内存
  {
    __buf[__len++] = *__beg;
    ++__beg;
  }
  _Rep *__r = _Rep::_S_create(__len, size_type(0), __a); // 分配内存
  _M_copy(__r->_M_refdata(), __buf, __len);              // 拷贝数据
  __try
  {
    while (__beg != __end) // 数据未拷贝完
    {
      if (__len == __r->_M_capacity) // 需要扩容
      {
        // Allocate more space.
        _Rep *__another = _Rep::_S_create(__len + 1, __len, __a);   // 分配内存
        _M_copy(__another->_M_refdata(), __r->_M_refdata(), __len); // 拷贝原内存数据
        __r->_M_destroy(__a);                                       // 销毁原结构
        __r = __another;                                            // 重新赋值结构
      }
      __r->_M_refdata()[__len++] = *__beg; // 添加新元素
      ++__beg;
    }
  }
  __catch(...)
...
basic_string<_CharT, _Traits, _Alloc> &basic_string<_CharT, _Traits, _Alloc>::assign(const basic_string &__str)
{
  if (_M_rep() != __str._M_rep())
  {
    ...
    _CharT *__tmp = __str._M_rep()->_M_grab(__a, __str.get_allocator()); // 另创内存
    _M_rep()->_M_dispose(__a);                                           // 处理原描述结构
    _M_data(__tmp);                                                      // 使用新内存
  }
  return *this;
}
...
basic_string<_CharT, _Traits, _Alloc> &basic_string<_CharT, _Traits, _Alloc>::assign(const _CharT *__s, size_type __n)
{
  ...
  if (_M_disjunct(__s) ||       // 数据区没有重叠
      _M_rep()->_M_is_shared()) // 多个引用
...
basic_string<_CharT, _Traits, _Alloc> &basic_string<_CharT, _Traits, _Alloc>::insert(size_type __pos, const _CharT *__s, size_type __n)
{
  ...
  {
    // Work in-place.
    const size_type __off = __s - _M_data(); // 记录偏移
    _M_mutate(__pos, 0, __n);                // pos处插入 处理扩容
    __s = _M_data() + __off;
    _CharT *__p = _M_data() + __pos;
    if (__s + __n <= __p) // 区域不重叠
      _M_copy(__p, __s, __n);
    else if (__s >= __p) //
...
template <typename _CharT, typename _Traits, typename _Alloc>
void basic_string<_CharT, _Traits, _Alloc>::reserve(size_type __res) // 向上扩充容量
{
...
typename basic_string<_CharT, _Traits, _Alloc>::_Rep *basic_string<_CharT, _Traits, _Alloc>::_Rep::_S_create(size_type __capacity, size_type __old_capacity, const _Alloc &__alloc)
{
  ...
  if (__capacity > __old_capacity && __capacity < 2 * __old_capacity)
    __capacity = 2 * __old_capacity; // 2倍扩容
