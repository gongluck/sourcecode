...
private:
  ...
  struct _Rep : _Rep_base // 字符串描述结构
  {
    ...
    static const size_type _S_max_size; // 最大容量
    static const _CharT _S_terminal;    // 结束符
    ...
    {
      return (!_M_is_leaked() && __alloc1 == __alloc2) // 是否可以拷贝
                 ? _M_refcopy()                        // 复制引用
                 : _M_clone(__alloc1);                 // 数据克隆
    }
...
#endif
        __gnu_cxx::__atomic_add_dispatch(&this->_M_refcount, 1); // 引用计数加一
      return _M_refdata();
    } // XXX MT
  ...
  struct _Alloc_hider : _Alloc // 隐藏构造器
  {
...
public:
  ...
  void push_back(_CharT __c)
  {
    ...
    if (__len > this->capacity() ||                    // 容量不够
        _M_rep()->_M_is_shared())                      // 写时拷贝
      this->reserve(__len);                            // 扩容/复制空间
    traits_type::assign(_M_data()[this->size()], __c); // 赋值添加元素
    _M_rep()->_M_set_length_and_sharable(__len);       // 设置长度和引用计数
  }
  ...
  {
    return _M_replace_aux(size_type(0), this->size(), __n, __c); // 范围赋值
  }
  ...
  {
    _GLIBCXX_DEBUG_PEDASSERT(__p >= _M_ibegin() && __p <= _M_iend()); // 范围检查
    const size_type __pos = __p - _M_ibegin();                        // 计算插入点
    _M_replace_aux(__pos, size_type(0), size_type(1), __c);           // 插入元素
    _M_rep()->_M_set_leaked();                                        // 设置引用计数 独享数据
    return iterator(_M_data() + __pos);                               // 返回新元素迭代器
  }
...
#if (defined(__GXX_EXPERIMENTAL_CXX0X__) && defined(_GLIBCXX_USE_C99) && !defined(_GLIBCXX_HAVE_BROKEN_VSWPRINTF))
