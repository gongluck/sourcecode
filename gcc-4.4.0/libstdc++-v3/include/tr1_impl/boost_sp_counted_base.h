...
namespace std
{
  ...
  public:
    ...
    void _M_weak_add_ref() // nothrow //增加管理块的引用计数
    {
      __gnu_cxx::__atomic_add_dispatch(&_M_weak_count, 1);
    }

    void _M_weak_release() // nothrow //释放管理块
    {
      if (__gnu_cxx::__exchange_and_add_dispatch(&_M_weak_count, -1) == 1)
      {
        if (_Mutex_base<_Lp>::_S_need_barriers)
    ...
    long _M_get_use_count() const // nothrow //获取资源引用计数
    {
  ...
  private:
    ...
    _Atomic_word _M_use_count;  // #shared //共享指针的引用计数 为0时析构托管对象
    _Atomic_word _M_weak_count; // #weak + (#shared != 0) //管理对象的引用计数 为0时销毁管理对象
  };
