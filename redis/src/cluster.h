...
typedef struct clusterLink
{
  ...
  int fd; /* TCP socket file descriptor */                                  //套接字描述符
  sds sndbuf; /* Packet send buffer */                                      //发送缓冲区
  sds rcvbuf; /* Packet reception buffer */                                 //接收缓冲区
  struct clusterNode *node; /* Node related to this link if any, or NULL */ //相关节点
} clusterLink;
...
typedef struct clusterNode
{
  ...
  char name[REDIS_CLUSTER_NAMELEN]; /* Node name, hex string, sha1-size */       //节点名字
  int flags; /* REDIS_NODE_... */                                                //节点标识
  uint64_t configEpoch; /* Last configEpoch observed for this node */            //用于故障转移的配置纪元
  unsigned char slots[REDIS_CLUSTER_SLOTS / 8]; /* slots handled by this node */ //节点处理的槽位
  int numslots; /* Number of slots handled by this node */                       //节点处理的槽数量
  int numslaves; /* Number of slave nodes, if this is a master */                //正在复制这个主节点的从节点数
  struct clusterNode **slaves; /* pointers to slave nodes */                     //正在复制这个主节点的从节点
  struct clusterNode *slaveof; /* pointer to the master node */                  //指向主节点
  ...
  char ip[REDIS_IP_STR_LEN]; /* Latest known IP address of this node */          //节点IP
  int port; /* Latest known port of this node */                                 //节点端口
  clusterLink *link; /* TCP/IP link with this node */                            //节点的网络相关信息
...
{
  clusterNode *myself; /* This node */                            //指向当前节点
  uint64_t currentEpoch;                                          //当前配置纪元
  int state; /* REDIS_CLUSTER_OK, REDIS_CLUSTER_FAIL, ... */      //集群状态
  int size; /* Num of master nodes with at least one slot */      //至少处理一个槽的节点数
  dict *nodes; /* Hash table of name -> clusterNode structures */ //集群中的节点
  dict *nodes_black_list;                                         /* Nodes we don't re-add for a few seconds. */
  clusterNode *migrating_slots_to[REDIS_CLUSTER_SLOTS];           //正在迁移到其他节点的槽
  clusterNode *importing_slots_from[REDIS_CLUSTER_SLOTS];         //正在从其他节点导入的槽
  clusterNode *slots[REDIS_CLUSTER_SLOTS];                        //记录所有槽的处理节点
