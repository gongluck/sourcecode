...
static void x264_predict_8x8_filter_c(pixel *src, pixel edge[36], int i_neighbor, int i_filters)
{
  ...
  if (i_filters & MB_LEFT)
  {
    ...
    edge[14] = ((have_lt ? SRC(-1, -1) : SRC(-1, 0)) + 2 * SRC(-1, 0) + SRC(-1, 1) + 2) >> 2;
    PL(1)
    PL(2)
    PL(3)
    PL(4) PL(5) PL(6)
  ...
  if (i_filters & MB_TOP)
  {
    ...
    edge[16] = ((have_lt ? SRC(-1, -1) : SRC(0, -1)) + 2 * SRC(0, -1) + SRC(1, -1) + 2) >> 2;
    PT(1)
    PT(2)
    PT(3)
    PT(4) PT(5) PT(6)
        edge[23] = (SRC(6, -1) + 2 * SRC(7, -1) + (have_tr ? SRC(8, -1) : SRC(7, -1)) + 2) >> 2;
...
static void x264_predict_8x8_hd_c(pixel *src, pixel edge[36])
{
  PREDICT_8x8_LOAD_TOP
      PREDICT_8x8_LOAD_LEFT
          PREDICT_8x8_LOAD_TOPLEFT int p1 = pack_pixel_1to2(F1(l6, l7), F2(l5, l6, l7));
...
{
  PREDICT_8x8_LOAD_LEFT int p1 = pack_pixel_1to2(F1(l0, l1), F2(l0, l1, l2));
