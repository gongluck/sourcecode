...
static int x264_validate_parameters(x264_t *h, int b_open)
{
  ...
  if (h->param.i_avcintra_class)
  {
    ...
    int type = h->param.i_avcintra_class == 200 ? 2 : h->param.i_avcintra_class == 100 ? 1
                                                  : h->param.i_avcintra_class == 50    ? 0
                                                                                       : -1;
    if (type < 0)
    {
    ...
    {
      if (h->param.i_width == 1440 && h->param.i_height == 1080)
        res = 1;
      else if (h->param.i_width == 960 && h->param.i_height == 720)
        res = 0;
    }
    else if (i_csp >= X264_CSP_I422 && i_csp < X264_CSP_I444 && type)
    {
      if (h->param.i_width == 1920 && h->param.i_height == 1080)
        res = 1;
      else if (h->param.i_width == 1280 && h->param.i_height == 720)
        res = 0;
    }
    else
    {
  ...
  if (b_open && (h->param.rc.i_rc_method == X264_RC_CQP || h->param.rc.i_rc_method == X264_RC_CRF) && h->param.rc.i_qp_constant == 0)
  {
    h->mb.b_lossless = 1;
...
x264_t *x264_encoder_open(x264_param_t *param)
{
  ...
  h->frames.b_have_lowres = !h->param.rc.b_stat_read && (h->param.rc.i_rc_method == X264_RC_ABR || h->param.rc.i_rc_method == X264_RC_CRF || h->param.i_bframe_adaptive || h->param.i_scenecut_threshold || h->param.rc.b_mb_tree || h->param.analyse.i_weighted_pred);
  ...
  {
    if (!strcmp(x264_cpu_names[i].name, "SSE") && h->param.cpu & (X264_CPU_SSE2))
      continue;
    if (!strcmp(x264_cpu_names[i].name, "SSE2") && h->param.cpu & (X264_CPU_SSE2_IS_FAST | X264_CPU_SSE2_IS_SLOW))
      continue;
    if (!strcmp(x264_cpu_names[i].name, "SSE3") && (h->param.cpu & X264_CPU_SSSE3 || !(h->param.cpu & X264_CPU_CACHELINE_64)))
      continue;
    if (!strcmp(x264_cpu_names[i].name, "SSE4.1") && (h->param.cpu & X264_CPU_SSE42))
      continue;
    if (!strcmp(x264_cpu_names[i].name, "BMI1") && (h->param.cpu & X264_CPU_BMI2))
      continue;
    if ((h->param.cpu & x264_cpu_names[i].flags) == x264_cpu_names[i].flags && (!i || x264_cpu_names[i].flags != x264_cpu_names[i - 1].flags))
  ...
  *h->reconfig_h = *h;
  ...
  const char *profile = h->sps->i_profile_idc == PROFILE_BASELINE ? "Constrained Baseline" : h->sps->i_profile_idc == PROFILE_MAIN  ? "Main"
        ...
        if (CHROMA444)
        {
          for (int i = 0; i < 4; i++)
            if (h->mb.i_cbp_luma & (1 << i))
              ...
              {
                int s8 = i * 4 + p * 16;
                int nnz8x8 = M16(&h->mb.cache.non_zero_count[x264_scan8[s8] + 0]) | M16(&h->mb.cache.non_zero_count[x264_scan8[s8] + 8]);
    ...
    if (!i_slice_num || !x264_frame_new_slice(h, h->fdec))
    {
      if (h->param.i_slice_max_mbs)
      {
        if (SLICE_MBAFF)
        {
          ...
          int last_mbaff = 2 * (h->sh.i_first_mb % h->mb.i_mb_width) + h->mb.i_mb_width * (h->sh.i_first_mb / h->mb.i_mb_width) + h->param.i_slice_max_mbs - 1;
...
#define SUM3b(p, o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
    ...
    int64_t i_skip = SUM3b(h->stat.i_mb_count, P_SKIP) + SUM3b(h->stat.i_mb_count, B_SKIP);
