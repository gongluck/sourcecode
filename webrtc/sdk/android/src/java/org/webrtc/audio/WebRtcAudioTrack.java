...
class WebRtcAudioTrack {
    ...
    public void run() {
      ...
      while (keepAlive) {
        ...
        if (bytesWritten != sizeInBytes) {
          Logging.e(
            TAG,
            "AudioTrack.write played invalid number of bytes: " + bytesWritten
          );
          ...
          if (bytesWritten < 0) {
            ...
            reportWebRtcAudioTrackError(
              "AudioTrack.write failed: " + bytesWritten
            );
          }
        }
        if (useLowLatency) {
        ...
        byteBuffer.rewind(); //更新缓冲区position
  ...
  private int initPlayout(
    ...
    if (bufferSizeFactor > 1.0) {
      useLowLatency = false; //缓冲因子大于1.0时不使用低延时模式
    }
  ...
  private boolean startPlayout() {
    ...
    audioThread = new AudioTrackThread("AudioTrackJavaThread"); //写数据线程
    audioThread.start();
    return true;
  }
  ...
  private static void logNativeOutputSampleRate(int requestedSampleRateInHz) {
    final int nativeOutputSampleRate = AudioTrack.getNativeOutputSampleRate(
  ...
  private static AudioAttributes getAudioAttributes(
    ...
    if (overrideAttributes != null) {
      ...
      if (
        overrideAttributes.getContentType() !=
  ...
  private static AudioAttributes.Builder applyAttributesOnQOrHigher(
    ...
    return builder.setAllowedCapturePolicy(
      overrideAttributes.getAllowedCapturePolicy()
    );
  }
  ...
  private void logBufferCapacityInFrames() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
      Logging.d(
        ...
        audioTrack.getBufferCapacityInFrames()
      );
    }
  }
  ...
  private static native void nativeCacheDirectBufferAddress(
    long nativeAudioTrackJni,
    ByteBuffer byteBuffer
  );

  private static native void nativeGetPlayoutData(
    long nativeAudioTrackJni,
    int bytes
  );
