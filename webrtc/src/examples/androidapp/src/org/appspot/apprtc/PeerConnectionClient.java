...
public class PeerConnectionClient {
  ...
  private static final String AUDIO_ECHO_CANCELLATION_CONSTRAINT =
    "googEchoCancellation";
  private static final String AUDIO_AUTO_GAIN_CONTROL_CONSTRAINT =
    "googAutoGainControl";
  private static final String AUDIO_HIGH_PASS_FILTER_CONSTRAINT =
    "googHighpassFilter";
  private static final String AUDIO_NOISE_SUPPRESSION_CONSTRAINT =
    "googNoiseSuppression";
  private static final String DTLS_SRTP_KEY_AGREEMENT_CONSTRAINT =
  ...
  public PeerConnectionClient(
    ...
    Log.d(
      TAG,
      "Preferred video codec: " + getSdpVideoCodecName(peerConnectionParameters)
    );
  ...
  public void createPeerConnectionFactory(
    ...
    if (factory != null) {
      throw new IllegalStateException(
        "PeerConnectionFactory has already been constructed"
      );
    }
  ...
  public void createPeerConnection(
    ...
    createPeerConnection(
      localRender,
      Collections.singletonList(remoteSink),
      videoCapturer,
      signalingParameters
    );
  }
  ...
  private void createPeerConnectionFactoryInternal(
    ...
    final boolean enableH264HighProfile = VIDEO_CODEC_H264_HIGH.equals(
      peerConnectionParameters.videoCodec
    );
    final VideoEncoderFactory encoderFactory;
    final VideoDecoderFactory decoderFactory;

    if (peerConnectionParameters.videoCodecHwAcceleration) { //硬件编解码
      encoderFactory =
  ...
  private void createPeerConnectionInternal() {
    ...
    if (dataChannelEnabled) {
      ...
      init.negotiated =
        peerConnectionParameters.dataChannelParameters.negotiated;
      init.maxRetransmits =
        peerConnectionParameters.dataChannelParameters.maxRetransmits;
    ...
    if (isVideoCallEnabled()) {
      peerConnection.addTrack(
        createVideoTrack(videoCapturer), //创建视频source和track
        mediaStreamLabels
      );
  ...
  private File createRtcEventLogOutputFile() {
    ...
    final String outputFileName =
      "event_log_" + dateFormat.format(date) + ".log";
    return new File(
            ...
            peerConnection.addIceCandidate(
              ...
              new AddIceObserver() {
                ...
                public void onAddFailure(String error) {
                  Log.d(
                    TAG,
                    "Candidate " + candidate + " addition failed: " + error
                  );
                }
              }
            );
      ...
      () -> {
        ...
        for (RtpParameters.Encoding encoding : parameters.encodings) {
          ...
          encoding.maxBitrateBps =
            maxBitrateKbps == null ? null : maxBitrateKbps * BPS_IN_KBPS;
        }
  ...
  private static String getSdpVideoCodecName(
  ...
  private void changeCaptureFormatInternal(
    int width,
    int height,
    int framerate
  ) {
    if (!isVideoCallEnabled() || isError || videoCapturer == null) {
      Log.e(
        TAG,
        "Failed to change capture format. Video: " +
        isVideoCallEnabled() +
        ". Error : " +
        isError
      );
