...
public class HardwareVideoEncoderFactory implements VideoEncoderFactory {
  ...
  private boolean isSupportedCodec(
    ...
    if (
      MediaCodecUtils.selectColorFormat(
        MediaCodecUtils.ENCODER_COLOR_FORMATS,
        info.getCapabilitiesForType(type.mimeType()) //编码器能力集合
      ) ==
      null
    ) {
      return false;
    ...
    return (
      //判断是否支持硬编码
      isHardwareSupportedInCurrentSdk(info, type) && isMediaCodecAllowed(info)
    );
  }
  ...
  private boolean isHardwareSupportedInCurrentSdkH264(MediaCodecInfo info) {
    ...
    if (H264_HW_EXCEPTION_MODELS.contains(Build.MODEL)) { //型号黑名单
      return false;
    }
    String name = info.getName();
    ...
    return (
      (
        name.startsWith(QCOM_PREFIX) && //OMX.qcom.开头
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT //Build.VERSION_CODES.KITKAT以上版本
      ) ||
      // Exynos H264 encoder is supported in LOLLIPOP or later.
      (
        name.startsWith(EXYNOS_PREFIX) && //OMX.Exynos.开头
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP //Build.VERSION_CODES.LOLLIPOP以上版本
      )
    );
  }
